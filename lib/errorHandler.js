...
module.exports = function (config) {
    ...
    return function errorHandler(err, req, res, next) {
        ...
        if (reportError === 'postMessage') {
            ...
            res.end('<!DOCTYPE html><html><head></head><script>' +
                        'if (parent && parent.postMessage) {' +
                            'window.onload = function () {' +
                                'parent.postMessage(\'' + JSON.stringify(errorObject).replace(/'/g, "\\'") + '\', location.origin || location.protocol + \'//\' + location.host + \'/\');' +
                            '}' +
                        '}' +
                    '</script><body></body></html>');
        } else if (reportError === 'jsonInHtml' || (req.is('multipart/form-data') && req.headers['x-requested-with'] !== 'XMLHttpRequest')) {
            ...
            if (res.headersSent) {
                return next(err); // defer to the default error handler
            }
